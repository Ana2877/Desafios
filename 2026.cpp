...
int main() {
  ...
  for(int i = 0; i < number_of_test_cases; i++) {
    int number_of_packets, weight_capacity, ornaments_on_package, package_weight;
    ...
    for (int j = 0; j < number_of_packets; j++) {
      cin >> ornaments_on_package >> package_weight;

      valores[j] = ornaments_on_package;
      pesos[j] = package_weight;
      quantidades[j] = 1;
    }

    cout << "Galho " << i+1 << ":\n" << "Numero total de enfeites: " <<  mochila(weight_capacity) << endl << endl;
  }
